...
def calculate_scores(info, stock):
    ...
    elif per is not None and isinstance(per, (int, float)):
        ratio = per / 20.0
        if ratio <= 0.5:
            val_score = 100
        elif ratio <= 0.8:
    ...
    elif ps is not None and isinstance(ps, (int, float)):
        ratio = ps / 5.0
        if ratio <= 0.5:
            val_score = 100
        elif ratio <= 0.8:
    ...
    try:
        ...
        if not hist.empty:
            ...
            mom_val = ((p_e - p_s) / p_s) * 100 if p_s != 0 else 0
    ...
    safe_score = 50
    safe_detail = "N/A"
    if de_raw is not None and isinstance(de_raw, (int, float)):
        de = de_raw
...
if submit_button:
    ...
    if tar:
        up = ((tar - cur) / cur) * 100 if cur else 0
